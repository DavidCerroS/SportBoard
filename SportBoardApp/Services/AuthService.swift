...
struct StoredStravaCredentials {
    let accessToken: String?
    let refreshToken: String?
    let expiresAt: Int?

    func hasUsableSession(now: Int = Int(Date().timeIntervalSince1970)) -> Bool {
...
final class AuthService: NSObject, ObservableObject, ASWebAuthenticationPresentationContextProviding {
    ...
    private func checkAuthenticationStatus() {
        isAuthenticated = storedCredentials().hasUsableSession()
    }

    private func storedCredentials() -> StoredStravaCredentials {
        StoredStravaCredentials(
            accessToken: keychain.readString(forKey: Constants.Keychain.accessTokenKey),
            refreshToken: keychain.readString(forKey: Constants.Keychain.refreshTokenKey),
            expiresAt: keychain.readInt(forKey: Constants.Keychain.expiresAtKey)
        )
    }
